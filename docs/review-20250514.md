...
1. **Label Parameters**: Global protocol types include label parameters (e.g., `TInteract<IO, Lbl, R, H, T>`) that serve for identification, debugging, and protocol verification.
...
1. **Empty Protocols**: Properly projects `TEnd<IO, Lbl>` to `EpEnd<IO, Me>`, though the label is lost.
