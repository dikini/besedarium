<!-- filepath: /home/dikini/Projects/besedarium/docs/projections-day2.md -->
...
- **Current Issue**: Our projection only handles the sender side of `TInteract`. When projecting
...
- **Complex Structure**: The n-dimensional output structure makes it tricky to update multiple
...
- Should we maintain the current approach with position-based updates, or switch to a more direct
structure?

- How do we best handle the receiving side of interactions? Options:
...
- What's the cleanest way to compose these projections when we add other combinators (TChoice,
TPar, TRec)?

## Next Steps

- Decide on whether to continue with current approach or switch to a different structure

- Implement proper bi-directional projection for TInteract:

  - Handle both sender and receiver
  - Maintain correct ordering of communications
  - Preserve protocol structure

- Add more compile-time assertions to verify projections:

  - Test multiple interactions
  - Verify both send and receive endpoints
  - Check ordering preservation

- Plan approach for other combinators:
...
- **Communication Events as Primary Structure**
